...
export function authoringEditContract(definition, grant, locks, fields, blocks, started) {
  ...
  return { addresses: allowed,
    ...
    values: {
      'goal/campaign': { description: 'Optional sustained optimization policy. mode optimize|improve; maxCandidates distinct from limits.iterations (proposal attempts); minExploration; 2–10 named families; allowedChanges prompt|settings|structure; screenCases/survivors/finalists/confirmationRepeats; validationQueries; concurrency 1–4; retests; repairCalls; reserveCalls/reserveUsd/estimatedCallUsd; unknownPricing pause|reserve; reuseLearning boolean. Requires fixed evaluation.baseline (including source for workflow targets), fixed references, no broad tools. Confirmation freezes a finalist; final cases never enter optimizer retrieval. Null retains acceptance-only behavior.' },
