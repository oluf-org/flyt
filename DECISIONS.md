...
| D84 | Work's run-status dropdown hides a development Agent debugger: a no-tool model diagnoses the durable session into evidence, probable cause, confidence, suspected block and editable retry guidance. Analysis is read-only; explicit retries use canonical block restart and preserve prior outputs for comparison. | Current; extends D55, D60 and D83 |
| D85 | Working-agent step limits are soft: warn and continue until the worker answers or is cancelled. Length-truncated worker responses continue in another logged query. Explicit hard bounds remain for small control turns, and parallel dispatch reports every failed child in a wave. | Current; refines D7, D8 and D83; bounded for generated workers by D86 |
| D86 | Unattended generated workers are hard-bounded. At the bound the harness withdraws every tool and grants a few answer-only turns to deliver from collected evidence; a worker that keeps requesting tools fails as incomplete. A refused or unconfinable shell result is never durable progress, shell tools are withheld when the execution world cannot confine commands, and a child session's approvals and questions are raised under the run the person launched. | Current; refines D82 and D85 |
| D87 | The chat is ONE window over named channels. A channel names its thread store, its tool ceiling, its grounding prompt, and how its turns’ proposals are read back; everything else about a turn is shared, so a third channel is a row in a table rather than a second chat. `loop` answers about the backlog and proposes tasks; `build` answers about the workflow the editor is drawing and proposes `stack:` commands, which a person applies through `ctx.commands`. Neither chat writes: a proposal is a card, and the commit is the human’s. | Current; extends D45 and D63 |
| D88 | D26’s ban on large backdrop blur is scoped to DECORATION. A modal that is about the surface behind it may blur that surface, because the alternative is a legible canvas people keep trying to click while a dialog is open. Ambient blur, animated mesh gradients, 3D tilt and skeleton shimmer remain out. | Current; scopes D26 |
