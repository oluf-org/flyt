...
export default function GoalWorkspace({ projectId, onOpenRun }) {
  ...
  const runAction = action => act(async () => { await invoke(action === 'start' ? 'start' : 'control', action === 'start' ? {} : { action }); await refresh(); });
  ...
  return <div className="goal-page"><header className="goal-header">
    <div className="goal-identity">
      ...
      {goal ? <strong role="status" className="goal-status">{goal.status.replaceAll('_', ' ')}</strong> : draft && <span className="goal-status quiet">{draft.approvedHash === draft.hash ? 'reviewed' : 'not reviewed'}</span>}
    ...
    {draft && <div className="goal-actions">
      <div className="goal-version-group">
        <label className="goal-version">Version<select aria-label="Definition revision" value={revisionView} onChange={event => { setRevisionView(event.target.value); setDetail(false); }}><option value="draft">{draft.revision}</option>{goal && <option value="running">running · {goal.activeRevision}</option>}</select></label>
      ...
      {goal?.live ? <button disabled={busy} onClick={() => runAction('pause')}>Pause</button> : <button className="goal-primary" disabled={busy || working || !!pendingProposals.length || !!goal?.pendingResult || (goal && !['ready', 'paused'].includes(goal.status) && !goal.recoverable && !(goal.status === 'failed' && goal.pendingRevision))} onClick={() => act(async () => { const next = await flush(); await checkRequirements(next); setReview(true); })}>{goal?.status === 'paused' ? 'Review and resume' : 'Review and start'}</button>}
    </div>}
  </header>
