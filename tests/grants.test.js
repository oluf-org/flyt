...
test('selectors resolve by what a tool IS, not by a list someone maintains', () => {
  ...
  assert.deepEqual([...expandRefs('trust:untrusted', ctx).ids].sort(), ['extract_page', 'scrape_page', 'web_fetch', 'web_search']);
  ...
  assert.deepEqual([...expandRefs('web', ctx).ids].sort(), ['extract_page', 'scrape_page', 'web_fetch', 'web_search']);
