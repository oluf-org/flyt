...
import {
  WORK, BUILD, GOALS, CHATS, LIBRARY, MODELS, HISTORY, DESTINATIONS, INITIAL, navigate, heading, hint, traceOf, state,
  adjacent,
} from '../src/v2/shellRouting.js';

test('Work, Build, Goals, Chats, Library, Models and Statistics are permanent destinations', () => {
  assert.deepEqual(DESTINATIONS, [WORK, BUILD, GOALS, CHATS, LIBRARY, MODELS, HISTORY]);
  ...
  assert.equal(DESTINATIONS.indexOf(LIBRARY), 4);
});
...
test('headings name every permanent surface, and only those', () => {
  ...
  assert.equal(heading(HISTORY), 'Statistics');
  assert.equal(heading(CHATS), 'Chats');
...
test('every destination is reachable from every other in one hop', () => {
  assert.deepEqual(adjacent({ dest: WORK, run: null }), [BUILD, GOALS, CHATS, LIBRARY, MODELS, HISTORY]);
  assert.deepEqual(adjacent({ dest: BUILD, run: null }), [WORK, GOALS, CHATS, LIBRARY, MODELS, HISTORY]);
  assert.deepEqual(adjacent({ dest: LIBRARY, run: null }), [WORK, BUILD, GOALS, CHATS, MODELS, HISTORY]);
