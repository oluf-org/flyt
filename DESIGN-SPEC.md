...
Chats is the project history destination, also reached from Work's Recent section and its conversation sidebar. It lists loop instances and workflow conversations by date, with search and type filters. A loop keeps one entry across all of its setup, iteration, repair and test runs; workflow follow-ups share their conversation identity. Loop entries open the saved goal directly and workflows open their recorded run. Loop icons use circular arrows; workflow entries retain their existing sigils.

Completed goals open on Results & stats. The result shows fixed checks, selectable iterations, the saved candidate, active execution time, model calls, recorded tokens and known cost. Usage is read from canonical child sessions and includes setup, repairs and candidate tests; unavailable measurements remain unknown. The Statistics destination has separate Loops and Models & workflows views. Loop aggregates count instances, preserve non-achieved outcomes, and exclude unfinished loops from the settled success-rate denominator.
...
Attended workflows may author one **Plan & dispatch** block when the task shape is not known until run time. It creates a validated task DAG and records the generated work as run-only child blocks. `No`, `Low`, `Medium`, and `High` are modes over the same block: No forces a serial chain; the other modes ask the planner to move from conservative to aggressive independence, while required producer edges and declared write-file conflicts remain serial in every mode. The dispatcher starts only ready tasks, under a bounded concurrency limit, and refills free slots after each task settles, and never rewrites the saved workflow with a run's generated plan. The planner is asked for the fewest tasks that keep each worker bounded, because every dependent task costs a full agent run and loses its predecessor's context. Each generated worker has a hard round bound: at the bound its tools are withdrawn and it must deliver from what it has read, so one looping worker cannot occupy a slot indefinitely. A child's approvals and questions surface under the launched run, and shell tools are withheld up front when the execution world cannot confine commands.
...
**Goals evaluation.** New Goal drafts expose versioned evaluation in Design, alongside Setup once and Each iteration. Runtime owns mandatory evaluation after the editable recipe. Experiments distinguish required gates, named measurements, subjective comparisons, observed sample counts, current/best/partial, final verification and benchmark versions. References have inspectable verification, automatic or manual promotion, and recorded restore transitions. Baseline comparisons use the selected attempt's benchmark version. Existing saved Goals retain their frozen legacy containment policy. This uses the current Goal canvas, authoring grants, design tokens, lifecycle controls and evidence views; it adds no second workflow editor or navigation surface. See `docs/robust-evaluator.md`.
