...
**Canonical stack source.** `StackStore` reads `stacks/<id>.stack.yaml`. An older `flows/<id>.flow.yaml` is converted in memory when opened and remains untouched until the first stack write. That write is parsed before it is accepted, creates the canonical stack file, and only then removes the old source. Layout sidecars are never migrated because `kernel/src/stack/layout.ts` derives geometry from containment.
