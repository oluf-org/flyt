...
A rejection at review is treated differently from every other failure, because it is the only one where the gates already passed: the work is sound and a reviewer has named one specific thing about it. The rejected commit is recorded on the task as `resumeFrom`, the next attempt's worktree starts from it, and the brief says so — so a stray file is a correction rather than a rebuild on a dearer model. A `resumeFrom` that no longer resolves degrades to the base branch and is cleared. Red gates and an empty diff clear it too: those say the attempt is wrong in a way a fresh start may fix.
