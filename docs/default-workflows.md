...
Filesystem confinement availability and Node piped-child support are probed separately. A filesystem probe success alone does not certify Node/npm commands requiring piped children. Workers receive any observed limitation; an actual failed command matching a failed capability probe ends verification instead of spending implementation repairs on the same infrastructure failure. No alternate command is credited as the requested command, and no sandbox permission is expanded automatically.
...
Windows includes a private-pipe adapter that applies the token default DACL to new named pipes without an explicit descriptor. Node's piped descendants retain the restricted token and existing filesystem denial; an invocation-specific IPC capability grants no filesystem access. With `FLYT_RELEASE_SANDBOX_E2E=1`, platform tests still require the unchanged npm/Node runner to pass. The Electron probe uses the installed platform executable on all three operating systems. Native Windows, Linux and macOS checks run in GitHub before publication; local Python fixtures alone do not establish Node or cross-platform compatibility.
