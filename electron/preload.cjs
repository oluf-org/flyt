...
const api = {
  ...
  readRunView: (pid, runId) => ipcRenderer.invoke('run:snapshot', pid, runId, { includeLog: true }),
  ...
  getRepoChanges: (pid, runId) => ipcRenderer.invoke('run:changes', pid, runId),
  openChangedFile: (pid, runId, relPath) => ipcRenderer.invoke('run:openChangedFile', pid, runId, relPath),
